    ...
    const EventCard = ({ event }: { event: EventData }) => {
        ...
        return (
            <div className="flex w-full bg-white shadow-md h-52 rounded-lg">
                <img className="object-cover rounded-l-lg" src={bannerImage} alt="banner" width={200} />
                <div className="flex flex-col py-4 px-6 justify-between">
                    <div className="flex flex-col gap-1">
                        <p className="text-2xl font-semibold break-words max-w-full">{event.name}</p>
                        <p className="text-sm">
                            {
                        ...
                        <p className="text-emerald-800 font-medium mt-1">
                            Event Date: {
                                `${startDate.getDate()} ${startDate.toLocaleString('default', { month: "long" })} ${startDate.getFullYear()}`
                            }
                        </p>
                    </div>
        ...
        <div className="px-16 py-4">
            ...
            <div className="mx-24">
                <Row gutter={[60, 60]} className="mt-6">
                    ...
                    <Col span={12}>
                        <div className="flex flex-col items-center justify-center w-full bg-emerald-50 rounded-2xl shadow gap-2 select-none h-80">
                            <div className="text-emerald-700 text-xl font-bold mb-3">Experience</div>
                            <Progress
                                type="dashboard"
                ...
                <div className="flex flex-col items-center w-full bg-emerald-50 rounded-2xl shadow gap-4 p-6 my-8">
                    <p className="text-gray-500 text-xl self-start font-semibold">Participated Events</p>
                    {events.map((event) => (

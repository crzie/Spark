...
const HomePage = () => {
    ...
    useEffect(() => {
        getAllEvents().then((eventData) => { setAllEvents(eventData) })
    }, [])
    ...
    return (
        <div className="h-full mb-10">
            ...
            {allEvents.map((document) => <EventDetailCard key={document.id} event={document.data} />)}
...
const EventDetailCard = ({ event }: { event: EventData }) => {
    ...
    console.log(new Date(event.eventStart))
    ...
    return (
        <div className='ml-16 mr-16 mt-10 gap-10 flex justify-between'>
            ...
            <div className='w-1/2 flex flex-col justify-center'>
                ...
                <p className='flex items-center gap-1'><FaCalendar />{event.eventStart + " - " + event.eventEnd}</p>
                <p className='flex items-center gap-0.5'> <img src={xpIcon} alt="" className='w-4 h-4' />{event.bounty}</p>
